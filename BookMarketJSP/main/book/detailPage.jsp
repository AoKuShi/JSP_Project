	...
	<% } else { %>
	   ...
	   <%
		...
		<% } else if (session.getAttribute("MemberId") != null){ %>
			<a href="<%= request.getContextPath() %>/main/cart/add.jsp?bookId=<%= book.getId() %>"><button>장바구니 담기</button></a>
		<% } %>
		<a href="<%= request.getContextPath() %>/main/index.jsp"><button>도서 목록</button></a>
	<% } %>
